...
반도체 산업은 크게 램(RAM)과 롬(ROM)으로 대표되는 <b>메모리반도체</b>와 CPU, GPU, 그리고 등의 <b>시스템반도체</b>로 나뉘는데,  
<b>SoC</b>는 대표적인 시스템반도체의 일종으로, CPU, GPU, 특히 모바일 프로세서의 경우 여기에 이미지 데이터 처리를 위한 ISP(Image Signal Processor), NPU(Neural Processing Unit) 외 VPU, DSP, 모뎀(MODEM)등까지, 전체 시스템이 칩 하나에 담겨있는 기술집약 반도체를 말한다.
